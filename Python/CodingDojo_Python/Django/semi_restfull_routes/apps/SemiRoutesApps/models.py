...
class UserManager(models.Manager):
    def validate(self, data):
        ...
        if len(data['first_name']) < 2:
            flag = False
            errs.append('First name cannot be less than 2 characters.')

        if not data ['first_name'].isalpha():
            flag = False
            errs.append('Last name cannot have numbers.')

        if len(data['last_name']) < 2:
            flag = False
            errs.append('Last name cannot be less than 2 characters')
