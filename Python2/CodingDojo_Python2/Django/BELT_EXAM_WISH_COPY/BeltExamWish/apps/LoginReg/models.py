...
class UserManager(models.Manager):
    def validate(self, data):
        ...
        if flag:
            ...
            user = User.objects.create(name = data['name'],alias = data['alias'], birth_date = data['birth_date'], password = hashed)
            # print data
    ...
    def l_process(self,data):
        flag = True
        errs = []
        suspect_user = User.objects.filter(alias=data['alias'])

        if suspect_user:
            hashed = User.objects.get(alias = data['alias']).password
